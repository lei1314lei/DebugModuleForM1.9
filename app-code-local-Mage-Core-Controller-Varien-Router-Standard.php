...
class Mage_Core_Controller_Varien_Router_Standard extends Mage_Core_Controller_Varien_Router_Abstract
{
    ...
    public function match(Zend_Controller_Request_Http $request)
    {
        ...
        $found = false;
	$timmer=0;
	$routerMatchingInfoes['a:RouterMatchingSuccessed']=false;
        foreach ($modules as $realModule) {
		$debugKey="c:".$timmer++."-RealModule($realModule)";
	    ...
	    $routerMatchingInfoes[$debugKey]["module"]=$realModule;
            $routerMatchingInfoes[$debugKey]["controllerClassName"]=$controllerClassName;
            ...
            $routerMatchingInfoes[$debugKey]['controllerInstance']=get_class($controllerInstance);
	    $routerMatchingInfoes[$debugKey]['action']=$action;
	    $routerMatchingInfoes[$debugKey]['hasAction']=false;
            if (!$controllerInstance->hasAction($action)) {
                continue;
            }
	    $routerMatchingInfoes[$debugKey]['hasAction']=true;
            $routerMatchingInfoes['a:RouterMatchingSuccessed']=true;
            $found = true;
            break;
        }
        Mage::dispatchEvent("router_match_after_dispath_action",array(
		"routerMatchingInfoes"=>$routerMatchingInfoes,
		    ));
        /**
         * if we did not found any suitable
         */
        if (!$found) {
	    $routerMatchingInfoes['a:RouterMatchingFailed']=true;
